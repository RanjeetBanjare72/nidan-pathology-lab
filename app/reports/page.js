...
export default function ReportsPage() {
  ...
  async function verifyReport() {
    if (!selected) return; const p = patient(selected), tests = testsOf(selected), results = resultsOf(selected);
    if (!p.name || !p.gender || !tests.length) { setMessage("Verify se pehle patient name, gender aur test required hain."); return; }
    for (const t of tests) for (let i = 0; i < paramsOf(t).length; i++) { const par = paramsOf(t)[i]; if (par?.calculated || par?.is_calculated) continue; if (valueOf(results, t, par, i, 0) === "") { setMessage(`${t?.name || "Test"}: ${par?.name || par?.parameterName || "Result"} result required hai.`); return; } }
    setBusy(true); setMessage(""); const now = new Date().toISOString();
    const { data, error } = await supabase.from("reports").update({ status: "verified", report_status: "Verified", verified_at: now, verified_by: null }).eq("id", selected.id).in("status", ["draft", "Pending", "Processing", "Ready"]).select("*").single();
    if (error) setMessage(error.message); else { setSelected(data); setReports((old) => old.map((r) => r.id === data.id ? data : r)); await supabase.from("report_audit_log").insert({ report_id: data.id, action: "verified", from_status: selected.status, to_status: "verified", details: {} }); }
    setBusy(false);
  }
  async function finalizeReport() {
    if (!selected || normalizeStatus(selected.status) !== "verified") return; setBusy(true); setMessage(""); const now = new Date().toISOString();
    const { data, error } = await supabase.from("reports").update({ status: "final", report_status: "Released", finalized_at: now, finalized_by: null, revision_number: selected.revision_number || 1 }).eq("id", selected.id).eq("status", "verified").select("*").single();
    if (error) setMessage(error.message); else { setSelected(data); setReports((old) => old.map((r) => r.id === data.id ? data : r)); await supabase.from("report_audit_log").insert({ report_id: data.id, action: "finalized", from_status: "verified", to_status: "final", revision_number: data.revision_number || 1, details: {} }); }
    setBusy(false);
  }
  async function createRevision() {
    if (!selected || normalizeStatus(selected.status) !== "final") return; const next = Number(selected.revision_number || 1) + 1; setBusy(true); setMessage("");
    const { error } = await supabase.from("report_revisions").insert({ report_id: selected.id, revision_no: next, action: "update", snapshot: reportData(selected), user_id: null, lab_id: selected.lab_id || null });
    if (!error) { await supabase.from("reports").update({ status: "draft", report_status: "Pending", revision_number: next, finalized_at: null, finalized_by: null }).eq("id", selected.id); await supabase.from("report_audit_log").insert({ report_id: selected.id, action: "revised", from_status: "final", to_status: "draft", revision_number: next, details: { reason: "Controlled revision" } }); const updated = { ...selected, status: "draft", report_status: "Pending", revision_number: next, finalized_at: null, finalized_by: null }; setSelected(updated); setReports((old) => old.map((r) => r.id === updated.id ? updated : r)); } else setMessage(error.message); setBusy(false);
  }
  return <main className="reportsPage"><style jsx>{` .reportsPage{min-height:100vh;background:#f3f7f9;color:#17212b;padding:20px;font-family:Arial,sans-serif}.top{display:flex;justify-content:space-between;align-items:center;gap:12px;margin-bottom:16px}.brand h1{margin:0;font-size:24px}.brand p{margin:4px 0 0;color:#64748b;font-size:12px}.controls,.actions{display:flex;gap:8px;flex-wrap:wrap}.controls input{padding:11px 13px;border:1px solid #d8e1e7;border-radius:8px;min-width:240px}.btn{border:1px solid #cbd5e1;background:white;border-radius:8px;padding:10px 14px;cursor:pointer;font-weight:700}.btn:disabled{opacity:.6;cursor:not-allowed}.btn.primary{background:#008f86;color:white;border-color:#008f86}.btn.warn{background:#fff7ed;border-color:#fed7aa}.btn.dark{background:#0b3340;color:white;border-color:#0b3340}.card{background:white;border:1px solid #e3eaee;border-radius:12px;box-shadow:0 3px 12px #0b33400d;overflow:hidden}.tableWrap{overflow:auto}.table{width:100%;border-collapse:collapse;font-size:12px}.table th{background:#f7fafb;text-align:left;color:#52606d;font-size:10px;padding:12px;border-bottom:1px solid #e5e7eb}.table td{padding:12px;border-bottom:1px solid #edf1f3}.status{display:inline-block;padding:5px 8px;border-radius:999px;font-size:10px;font-weight:800}.draft{background:#fff7ed;color:#c2410c}.verified{background:#eff6ff;color:#1d4ed8}.final{background:#ecfdf5;color:#15803d}.preview{margin-top:20px;padding:18px}.previewHead{display:flex;justify-content:space-between;align-items:center;gap:10px;border-bottom:2px solid #0f8f86;padding-bottom:12px}.lab h2{margin:0}.lab p{margin:3px 0;font-size:11px;color:#64748b}.info{display:grid;grid-template-columns:repeat(4,1fr);gap:1px;background:#dce5e8;margin:16px 0}.info>div{background:white;padding:10px}.label{font-size:9px;color:#64748b;text-transform:uppercase}.value{font-weight:700;font-size:12px}.section{margin-top:16px}.section h3{background:#eaf7f5;padding:9px;margin:0;font-size:14px}.reportTable{width:100%;border-collapse:collapse;font-size:11px}.reportTable th,.reportTable td{padding:8px;border:1px solid #dbe4e7}.reportTable th{background:#f5f8f9}.abnormal{font-weight:800;color:#dc2626}.locked{padding:9px;background:#ecfdf5;border:1px solid #bbf7d0;border-radius:8px;color:#166534;font-weight:700}.msg{margin:10px 0;padding:10px;background:#fff7ed;border:1px solid #fed7aa;border-radius:8px;color:#9a3412;font-size:12px}@media(max-width:700px){.top,.previewHead{flex-direction:column;align-items:stretch}.controls{flex-direction:column}.controls input{min-width:0}.info{grid-template-columns:repeat(2,1fr)}} @media print{.top,.table,.msg,.previewHead .actions{display:none!important}.reportsPage{padding:0;background:#fff}.preview{box-shadow:none;border:0;margin:0}} `}</style>
