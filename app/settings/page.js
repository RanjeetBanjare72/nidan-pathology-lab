...
export default function SettingsPage() {
  ...
  function updateSetting(key, value) { setSettings((previous) => ({ ...previous, [key]: value })); }
  ...
  async function handleLetterheadUpload(event) {
    ...
    try {
      ...
      if (message.toLowerCase().includes("row-level") || message.toLowerCase().includes("policy") || message.toLowerCase().includes("permission")) setStatus("Upload permission blocked hai. Login/authentication aur Supabase Storage policy check karein.");
      else if (message.toLowerCase().includes("bucket") || message.toLowerCase().includes("not found")) setStatus("Supabase Storage bucket 'nidan-assets' nahi mila.");
  ...
  return (
    <div className="settingsPage">
      ...
      <section className="card"><CardTitle icon="🏥" title="Laboratory Information" text="Ye information Final Report, Saved Reports aur PDF me use hogi." /><div className="grid"><Field label="Laboratory Name" value={settings.labName} onChange={(v) => updateSetting("labName", v)} placeholder="NIDAN PATHOLOGY LAB" /><Field label="Registration Number" value={settings.registrationNo} onChange={(v) => updateSetting("registrationNo", v)} placeholder="Lab registration number" /><Field full label="Laboratory Slogan" value={settings.slogan} onChange={(v) => updateSetting("slogan", v)} placeholder="Accurate Diagnosis • Trusted Care • Better Health" /><Field full textarea label="Laboratory Address" value={settings.labAddress} onChange={(v) => updateSetting("labAddress", v)} placeholder="Complete laboratory address" /><Field label="Mobile / Phone" value={settings.phone} onChange={(v) => updateSetting("phone", v)} placeholder="Mobile number" /><Field label="Email" type="email" value={settings.email} onChange={(v) => updateSetting("email", v)} placeholder="Lab email" /><Field full label="Medical / Reporting Doctor" value={settings.doctorName} onChange={(v) => updateSetting("doctorName", v)} placeholder="Doctor name" /></div></section>
      <section className="card"><CardTitle icon="🖼️" title="Logo / Letterhead" text="Letterhead ab browser localStorage ke bajay Supabase Storage me permanently save hoga." /><div className="uploadBox"><input type="file" accept="image/png,image/jpeg,image/jpg,image/webp" onChange={handleLetterheadUpload} disabled={uploading} /><small>PNG, JPG, JPEG ya WEBP • Maximum 2 MB</small></div>{settings.letterhead ? <div className="preview"><div className="previewHead"><strong>Uploaded Logo / Letterhead</strong><button type="button" onClick={removeLetterhead} disabled={uploading}>Remove</button></div><div className="previewImage"><img src={settings.letterhead} alt="NIDAN laboratory logo or letterhead" /></div><div className="stored">Supabase Storage URL saved successfully.</div></div> : <div className="empty"><div>🖼️</div><strong>No Logo / Letterhead Uploaded</strong><small>Report header me use karne ke liye logo upload karein.</small></div>}</section>
      <section className="card"><CardTitle icon="📋" title="Report Settings" text="Final aur Saved Report ke display options." /><div className="options"><SettingToggle title="Report Header" description="Report ke top par laboratory header show karein." checked={settings.reportHeader} onChange={(v) => updateSetting("reportHeader", v)} /><SettingToggle title="Show Laboratory Logo" description="Final aur Saved Report me laboratory logo display karein." checked={settings.showLogo} onChange={(v) => updateSetting("showLogo", v)} /><SettingToggle title="Show Reference Range" description="Report me reference range display karein." checked={settings.showReferenceRange} onChange={(v) => updateSetting("showReferenceRange", v)} /><SettingToggle title="Show Result Flag" description="LOW / HIGH / NORMAL flag show karein." checked={settings.showFlag} onChange={(v) => updateSetting("showFlag", v)} /><SettingToggle title="Auto Save Results" description="Result entry ke dauran data automatically save karein." checked={settings.autoSave} onChange={(v) => updateSetting("autoSave", v)} /></div></section>
      <section className="card"><CardTitle icon="⚙️" title="System Settings" text="Laboratory software ke general controls." /><div className="systemGrid"><SystemItem title="Test Master" description="Tests, parameters, units aur reference ranges manage karein." href="/tests" /><SystemItem title="Doctors" description="Referring doctors add aur manage karein." href="/doctors" /><SystemItem title="Patients" description="Registered patients manage karein." href="/patients" /><SystemItem title="Reports" description="Final laboratory reports dekhein aur print karein." href="/reports" /></div></section>
