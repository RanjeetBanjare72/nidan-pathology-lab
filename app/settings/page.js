...
export default function SettingsPage() {
  ...
  async function handleLetterheadUpload(event) {
    ...
    try {
      ...
      if (message.toLowerCase().includes("row-level") || message.toLowerCase().includes("policy") || message.toLowerCase().includes("permission")) {
        ...
        setStatus(`Letterhead upload nahi ho paya: ${message}`);
      }
    } finally {
      setUploading(false);
      event.target.value = "";
  ...
  async function removeLetterhead() {
    ...
    try {
      const marker = `/storage/v1/object/public/${BUCKET}/`;
      const oldPath = settings.letterhead.includes(marker)
        ? decodeURIComponent(settings.letterhead.split(marker)[1] || "")
        : "";
  ...
  return (
    <div className="settingsPage">
      <div className="settingsHeader">
        <div>
          <div className="smallTitle">LABORATORY SETTINGS</div>
          <h1>Settings</h1>
          <p>NIDAN Pathology Lab ki laboratory configuration manage karein.</p>
        </div>
        <div className="systemStatus"><span /> System Active</div>
      </div>

      {status && <div className="statusBar">{status}</div>}

      <section className="card">
        <div className="cardTitle"><div className="icon">🏥</div><div><h2>Laboratory Information</h2><p>Ye information Final Report, Saved Reports aur PDF me use hogi.</p></div></div>
        <div className="grid">
      ...
      <section className="card">
        <div className="cardTitle"><div className="icon">🖼️</div><div><h2>Logo / Letterhead</h2><p>Letterhead ab browser localStorage ke bajay Supabase Storage me permanently save hoga.</p></div></div>
        <div className="uploadBox">
          <input type="file" accept="image/png,image/jpeg,image/jpg,image/webp" onChange={handleLetterheadUpload} disabled={uploading} />
          <small>PNG, JPG, JPEG ya WEBP • Maximum 2 MB</small>
        </div>
        {settings.letterhead ? (
          <div className="preview">
            <div className="previewHead"><strong>Uploaded Logo / Letterhead</strong><button type="button" onClick={removeLetterhead} disabled={uploading}>Remove</button></div>
            <div className="previewImage"><img src={settings.letterhead} alt="NIDAN laboratory logo or letterhead" /></div>
            <div className="stored">Supabase Storage URL saved successfully.</div>
          </div>
        ) : (
          <div className="empty"><div>🖼️</div><strong>No Logo / Letterhead Uploaded</strong><small>Report header me use karne ke liye logo upload karein.</small></div>
        )}
      </section>

      <section className="card">
        <div className="cardTitle"><div className="icon">📋</div><div><h2>Report Settings</h2><p>Final aur Saved Report ke display options.</p></div></div>
...
function SettingToggle({ title, description, checked, onChange }) {
  return (
    <div className="toggleItem">
      ...
      <button type="button" className={checked ? "toggle active" : "toggle"} onClick={() => onChange(!checked)} aria-label={title} aria-pressed={checked}><span /></button>
    </div>
  );
}

function SystemItem({ title, description, href }) {
  return <div className="systemItem"><div><strong>{title}</strong><small>{description}</small></div><a href={href}>Open →</a></div>;
}
